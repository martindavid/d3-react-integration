...
class Sunburst extends React.Component {
  ...
  update(root, firstBuild, svg, partition, hueDXScale, x, y, radius, arc, node, self) {  // eslint-disable-line
    if (firstBuild) {
      ...
      const { tooltipContent } = self.props;
      ...
      svg.selectAll('path')
        ...
        .on('mouseover', function mouseover(d) {
          if (self.props.tooltip) {
            ...
            tooltip.html(() => {
              const name = utils.formatNameTooltip(d); return name;
            });
        ...
        .on('mousemove', () => {
          if (self.props.tooltip) {
            tooltip
              ...
              .style('left', `${self.props.tooltipPosition === 'right' ?
                d3.event.pageX - 100 : d3.event.pageX - 50}px`);
          }
          return null;
        })
